...
public class DetectionMarkers : MonoBehaviour
{
	...
	void InitArucoParameters()
    {
		...
		detectorParameters.CornerRefinementMinAccuracy = 0.00001; //found no effect
	...
	private void UpdatingPoses()
	{
		for (int i = 0; i < markersToUpdateLeft.Length; i++)
		{
			if (markersToUpdateLeft[i])
			{
				...
				GetObjectNewTransform(tvecLeft[i], rotMatLeft[i], out Vector3 worldPos, out Quaternion worldRot); //estimating worldspace position
